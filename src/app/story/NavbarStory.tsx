...
const NavbarStory =({storyId, CurrentUserFirstName,
CurrentUserLastName,CurrentUserId}: Props)=> {
    ...
    return (
        <div className='px-8 py-2 border-b-[1px]'>
            <div className='flex items-center justify-between'>
                ...
                hover:opacity-100 duration-100 ease-in cursor-pointer
                bg-green-600 hover:bg-green-700 rounded-full px-3 py-100
                text-[13px] text-white">Publish</button>
            ...
            {showPopup && (
                <SaveStoryPopUp storyId={storyId} PublishStory=
                {PublishStory} setShowPopUp={setShowPopup}
...
CurrentUserFirstName,CurrentUserId,CurrentUserLastName}
:SaveStoryPopUptypes) => {
    ...
    return(
        <div className="fixed bg-gray-50 w-full z-20
        overflow-auto top-0 left-0 right-0 bottom-0">
            ...
            <div className="max-w-[900px] mx-auto md:mt-28
            mt-10 grid md:grid-cols-2 grid-cols-1 gap-14">
                <div className="max-md:hidden">
                    <p className="font-semibold"> Story Preview
                    </p>
                    <div className="w-full h-[250px]
                    bg-gray-100 rounded my-3 border-b-[1px]">
                        {imgSrc && (
                            <Image src={imgSrc} width={250}
                            height={250} alt="Preview Image"
                            className="w-full h-full object-cover"/> 
                        )}

                    </div>
                    <h1 className="border-b-[1px] text-[18px]
                    font-semibold py-2">{h1elementwithouttag}</h1>
                    <p className="border-b-[1px] py-2 text-sm
                    text-neutral-500 pt-3">{first10Words}</p>
                    <p className="font-medium text-sm
                    pt-2">Note: <span className="font-normal
                    text-neutral-500">Changes here will affect
                    how your story appears in public places
                    like TechTalks's homepage and in subscribers'
                    inboxes - not the contents of the story
                    itself.</span></p>
                </div>
                <div>
                    <p className="py-2">Publishing to: <span>
                    {CurrentUserFirstName} {CurrentUserLastName}
                    </span></p>
                    <p className="text-sm pb-3 pt-1">Add or 
                    change topics (up to5) so readers know 
                    ...
                    (selectedtopics)} className="px-4 py-2
                    bg-green-600 hover:bg-green-700
                    rounded-full text-white text-sm mt-8">
                        Publish now
                    </button>
