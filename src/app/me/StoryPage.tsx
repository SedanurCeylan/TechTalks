    ...
    return (
        <div className='max-w-[1000px] mx-auto mt-12'>
            ...
            <div className='flex items-center space-x-6 border-b-[1px] mt-2'>
                <Link href='/me/drafts' className={`${path === 'drafts' ? "border-b-[1px] border-neutral-800 opacity-100" : "opacity-60"}
                text-sm pb-4`}> drafts {TotalDrafts}</Link>
                <Link href='/me/published' className={`${path === 'published' ? "border-b-[1px] border-neutral-800 opacity-100" : "opacity-60"}
                text-sm pb-4`}> published {TotalPublished}</Link>
                <Link href='/me/saved' className={`${path === 'saved' ? "border-b-[1px] border-neutral-800 opacity-100" : "opacity-60"}
                text-sm pb-4`}> saved {TotalSaved}</Link>
            </div>
            {path === 'drafts' && (
                <div className='mt-5'>
                    {stories.map((story) => {
                            ...
                            <Link key={story.id} href={`/story/${story.id}`}>
                                <div className='py-5'>
                                    ...
                                    <div className='flex items-center space-x-5'>
                                        <span className='text-sm opacity-60'>{new Date(story.updatedAt).toDateString()}
                                        </span>
                                    </div>
                                </div>
                            </Link>
                        ...
                        return (
                            <div key={story.id} className='py-5'>
                                ...
                                <div className='flex items-center space-x-5'>
                                    ...
                                    <button onClick={() => EditStory(story.id)}
                                        className='px-5 py-1 text-sm rounded-full bg-gray-500 text-white'>
                                        Edit
                                    </button>
                                </div>
                            </div>
                        ...
                        return (
                            ...
                            pb-10 border-neutral-100'>
                                ...
                                gap-10 grid-cols-2'>
                                    ...
                                    px-2 py-1 rounded-full text-[13px]'>
                                                    {story.topics}</span>
                                            )}
                                        </div>
                                    </div>
 ...
 export const AuthorDetail = ({story}:{story:Story}) => {
    ...
    return (
        <div className='flex items-center space-x-2'>
            <Image className='rounded-full' src={user?.imageUrl ?
                user.imageUrl : '/no-image.jpg'} width={24} height={24} alt='User Image' />
            <p className='text-sm'>
                {user?.firstName}
                {user?.lastName}.</p> 
            <p className='text-sm opacity-60'>
                {new Date(story.updatedAt).
                    toDateString().split(' ').slice(1, 4).join(' ')}</p>
        </div>
    )
}